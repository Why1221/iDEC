...
void LinList::insert(Linkable *f) {
  ...
  sd->next = first;
  sd->prev = NULL;
  if (first != NULL) first->prev = sd;

  // first, last und anz berichtigen
  anz++;
  first = sd;
  if (last == NULL) last = sd;
...
// liefert das i-te Element in der Liste
{
  ...
  if (ahead) {
    for (j = akt_index; j < i; j++) {
      if (!akt) error("LinList::get: List seems to be inkonsistent", TRUE);
      akt = akt->next;
    }
  } else {
    for (j = akt_index; j > i; j--) {
...
void LinList::print() {
  ...
  while (cur != NULL) {
    // printf("%d %f %f %f %f\n", cur->d->son, cur->d->bounces[0],
    //        cur->d->bounces[1], cur->d->bounces[2], cur->d->bounces[3]);
...
void LinList::check() {
  ...
  if (old_f == NULL) {
    ...
    if (anz != 0) error("LinList::check: first == NULL, anz != 0", FALSE);
    return;
  }
...
SortedLinList::SortedLinList() : LinList() { increasing = TRUE; }

void SortedLinList::set_sorting(bool _increasing) { increasing = _increasing; }
  ...
  while (any) {
    ...
    for (akt = first; akt != NULL; akt = akt->next) {
      ...
      if (old_akt != NULL) {
        swap = FALSE;
        if (_increasing) {
          if ((akt->d->son) > (old_akt->d->son)) swap = TRUE;
        } else {
          if ((akt->d->son) < (old_akt->d->son)) swap = TRUE;
        }
