...
// Please do not change the order; otherwise, confliction might happen
#include "ptR-tree.h"
#include "idec.h"
#include "gendef.h"

using namespace std;
using namespace AnnResults;

/*****************************************************************
builds IDEC tree(s) by incremental insertions
...
int build(char *_dfname, char *_ffolder, int _n, int _d, int _B, int _L,
  ...
  printf("Build IDEC trees.\n\n");

  idec = new IDEC();
  idec->init(_d, _n, _B, _L, _m);

  std::string rf = std::string(_ffolder) + "/iDEC-indexing.txt";

  HighResolutionTimer timer;
  AnnResultWriter writer(rf);
  writer.writeRow("s", "dsname,n,d,B,l,m,construction-time(us)");

  timer.restart();
  auto failed = idec->buildFromFile(_dfname, _ffolder);
  auto exec_time = timer.elapsed();
  writer.writeRow("siiiiif", _dfname, _n, _d, _B, _L, _m, exec_time);
...
/*****************************************************************
continue builds a idec tree/forest by incremental insertions
*****************************************************************/
...
int ctbuild(char *_dfname, char *_ffolder, int _n, int _d, int _B, int _L,
  ...
  if (idec->ctBuildFromFile(_dfname, _ffolder)) {
    ret = 1;
    goto recycle;
  }
...
int buildBin(char *_dfname, char *_ffolder, int _n, int _d, int _B) {
  ...
  idec = new IDEC();
  idec->init(_d, _n, _B, 1, 1);

  if (idec->buildBinFromFile(_dfname, _ffolder)) {
    ret = 1;
    goto recycle;
  }
...
int seqscan(char *_dfname, int _n, int _d, char *_qfname, int _k,
  ...
  for (int i = 0; i < 5; ++i) {
    for (int j = 0; j < enc_dim; ++j)
      fprintf(stdout, "%llu ", ds[i * enc_dim + j]);
    fprintf(stdout, "\n");
  }
  ...
  while (!feof(qfp)) {
    ...
    for (i = 0; i < enc_dim; i++) fscanf(qfp, " %lu", &(q[i]));
    fscanf(qfp, "\n");
  ...
  while (!feof(qfp)) {
    ...
    for (i = 0; i < _k; i++) knndist[i] = (float)MAXREAL;
    ...
    for (i = 0; i < _n; i++) {
      ...
      for (j = 0; j < _k; j++) {
        if (compfloats(dist, knndist[j]) == -1) break;
      }

      if (j < _k) {
        for (ii = _k - 1; ii >= j + 1; ii--) knndist[ii] = knndist[ii - 1];
...
int idecknn(char *_ffolder, int _numTrees, char *_rfname, char *_qfname, int _k,
  ...
  unsigned enc_dim = 0;
  IDEC *idec = NULL;
  IDEC_Hentry *rslt = NULL;

  HighResolutionTimer timer;
  AnnResultWriter writer(_ofname);
#ifdef MEASURE_IO_TIME
  std::string g_header = std::string(_DEFAULT_HEADER_E_) + ",io_time,raw_io";
  std::string g_fmt = std::string(_DEFAULT_FMT_E_) + "fi";
  writer.writeRow("s", g_header.c_str());
#else
  writer.writeRow("s", _DEFAULT_HEADER_E_);
#endif

  idec = new IDEC();

  if (idec->restore(_ffolder, _start_id, _numTrees)) {
    ret = 1;
    goto recycle;
  }
  ...
  for (i = 0; i < qn; i++) {
    ...
    for (j = 0; j < maxk; j++) fscanf(rfp, "%f", &(R[i * maxk + j]));
  }

  rslt = new IDEC_Hentry[_k];
  ...
  assert(idec->d % 64 == 0 && "Only support mutiples of 64");

  enc_dim = idec->d / 64;
  q = new uint64_t[idec->d];
  ...
  while (!feof(qfp)) {
    if (c == qn) {
      printf(
          "Query file longer than expected. I thought there were %d queries "
          "only.\n",
          qn);
    ...
    for (i = 0; i < enc_dim; i++) fscanf(qfp, "%llu", &(q[i]));

    fscanf(qfp, "\n");

    timer.restart();
    thiscost =
        idec->knn(q, _k, rslt, _start_id, _numTrees, _c, _tau, _t, early_stop);
    auto exec_time = timer.elapsed();
...
// thisOverallRatio += p;
#ifdef MEASURE_IO_TIME
      writer.writeRow(g_fmt.c_str(), c, i, rslt[i].id,
                      int(round(rslt[i].dist * rslt[i].dist)),
                      int(round(R[c * maxk + i] * R[c * maxk + i])), p *p,
                      exec_time, thiscost, FLAGS_G_IO_TIME, FLAGS_G_RIO_CNT);
#else
      writer.writeRow(_DEFAULT_FMT_E_, c, i, rslt[i].id,
                      int(round(rslt[i].dist * rslt[i].dist)),
                      int(round(R[c * maxk + i] * R[c * maxk + i])), p *p,
                      exec_time, thiscost);
#endif
...
void usage() {
  ...
  // printf("-l {value}\tnumber of idec-trees in the index (default 1)\n");
  printf("-n {value}\tcardinality\n");
  printf("-m {value}\tdimensionality of the projected space\n");
  printf("-f {string}\tfolder for idec index\n");
  ...
  printf(
      "-tau {value}\tthreshold of early termination condition (default "
      "0.1809)\n");
...
int main(int argc, char **argv) {
  ...
  if (!failed) {
    ...
    if (dfname[0] != '\0' && ffolder[0] != '\0' && n != -1 && d != -1 &&
      ...
      idecknn(ffolder, nt, rfname, qfname, k, ofname, startID, ratio, tau, t,
              early_stop);
      goto end;
    } else {
      failed = true;
