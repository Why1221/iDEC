...
void RTNode::init(RTree *_rt, int _block) {
  ...
  // here it is very important to get the level of the node first before
  // initalizing the entries
...
int RTNode::choose_subtree(Entry *_e) {
  ...
  int16_t *mbr = new int16_t[2 * dimension];
  memcpy(mbr, _e->bounces, 2 * dimension * sizeof(int16_t));
  ...
  {
    //    	fmin = MAXREAL;
    fmin = area(dimension, entries[inside[0]]->bounces);
    minindex = 0;
  ...
  // choose the one for which insertion causes the minimun overlap if
  // son_is_data else choose the one for which insertion causes the minimun area
  // enlargement
  {
    if (level == 1) // son_is_data
    {
      ...
      for (i = 0; i < num_entries; i++) {
        ...
        for (j = 0; j < num_entries; j++) {
          if (j != i) {
            old_o +=
                overlap(dimension, entries[i]->bounces, entries[j]->bounces);
        ...
        if (tmp != NULL) {
          ...
          if (del_ret != NOTFOUND) {
            ...
            case NORMAL:
              ...
              int16_t *mbr;
              mbr = succ->get_mbr();
              // memcpy(entries[i]->bounces, mbr, sizeof(float) * 2 *
              // dimension);
              memcpy(entries[i]->bounces, mbr, sizeof(int16_t) * 2 * dimension);
              dirty = true;
              delete[] mbr;
  ...
  } else // it's a leaf
  {
    for (int i = 0; i < num_entries; i++) {
      if (entries[i]->check_equal(_e)) {
        ...
        if (this != my_tree->root_ptr &&
            num_entries < (int)ceil(0.4 * capacity)) {
...
bool RTNode::FindLeaf(Entry *_e) {
  ...
  if (level > 0) {
    for (int i = 0; i < num_entries; i++) {
      //float *f;
      int16_t *f = overlapRect(my_tree->dimension, entries[i]->bounces, _e->bounces);
      if (f != NULL) {
        delete[] f;
        succ = entries[i]->get_son();
        bool find;
...
R_OVERFLOW RTNode::insert(Entry *_d, RTNode **_sn) {
  ...
  if (level > 0) // direcrtory node
  {
    if (level > _d->level) {
      ...
      memcpy(entries[follow]->bounces, mbr, sizeof(int16_t) * 2 * dimension);
      delete[] mbr;
  ...
  } else // data (leaf) node
  {
    ...
    // for the algorithms are more easy then
    {
      ...
      //-----------By TAO Yufei
      {
        ...
        for (int i_tmp = 0;i_tmp < capacity;++ i_tmp) delete entries[i_tmp];
        delete[] entries;
        entries = new_entries;
...
//	if (level > 0)
//	{
//		float *minmax_distanz;		// Array fuer MINMAXDIST aller
//Eintr"age 		int *indexliste;		// Liste (for Sorting and Prunching) 		int
//i,j,k,last,n; 		float akt_min_dist;		// minimal distanz computed upto
//now 		float minmaxdist,mindist;
//
//		BranchList *activebranchList;
//
//		n = num_entries;
//
//		//k = res->get_num(); 	// wird haben eine
//k-nearest-Narbor-Query
//
//		//*nearest_distanz = res->get(k-1)->distanz;  // der aktuell
//letzte
//													//
//n"achste Nachbar wird
//													//
//versucht zu ersetzen. 		if (res -> get_num() > 0)
//		{
//			if (*nearest_distanz != res -> get_first() -> distanz)
//			{
//				printf("testing...\n");
//				*nearest_distanz = res -> get_first() ->
//distanz;
//			}
//		}
//
//		activebranchList = new BranchList [n]; // Array erzeugen mit n
//Elementen
//
//		for( i = 0; i < n; i++)
//		{
//			activebranchList[i].entry_number = i;
//			activebranchList[i].minmaxdist =
//MINMAXDIST(QueryPoint,entries[i]->bounces); 			activebranchList[i].mindist =
//MINDIST(QueryPoint,entries[i]->bounces, dimension);
//		}
//
//		// sortbranchList
...
//		for( i = 0; i < last; i++)
//		{
//			entries[activebranchList[i].entry_number]->get_son()->NNSearch(QueryPoint,
//res, nearest_distanz); 			entries[i].del_son();
//
//			last =
//pruneBrunchList(nearest_distanz,activebranchList,last);
//		}
//
...
//			//i change this line to cater for both points and
//rectangles 			distanz = MINDIST(QueryPoint,entries[i]->bounces, dimension);
//
...
int RTNode::rangeQuery(int16_t *_mbr, SortedLinList *_res, bool _ids_wanted) {
  int i, n;
  int ret = 0;
  SECTION s;
  RTNode *succ;
...
//"mbr" is an array of MBRs. When the function finishes, "array" distribution
//will be filled with the indexes of the entries in the node, sorted according
// to the decided split manner. Furthermore, the function returns a value v which
// indicates that entries corresponding to the first v elements in "distribution"
// will be put in the first node, and the others in the second node.

// For example, assume that the node being split has 4 entries, whose indexes in
// the "entries" array are 0, ..., 4 respectively. Assume that this function
// yields the following "distribution" array:

//{4, 0, 2, 3, 1}

// and returns a value 3. It means that the entries with indexes 4, 0, 2 should
// be put in the first node, and the others in the second.

// int RTNode::mbr_split(float **_mbr, int **_distribution)
int RTNode::mbr_split(int16_t **_mbr, int **_distribution) {
  bool lu = FALSE;
  int i, j, k, l, s, n, m1, dist, split_axis = 0;
  SortMbr *sml, *smu;
  // float minmarg, marg, minover, mindead, dead, over, *rxmbr, *rymbr;
  int16_t *rxmbr, *rymbr;
  float minmarg, minover, mindead, marg, dead, over;
  ...
  rxmbr = new int16_t[2 * dimension];
  rymbr = new int16_t[2 * dimension];
...
void RTNode::split(RTNode *_sn) {
  ...
  for (i = 0; i < capacity; i++) {
    new_entries1[i] = new_one_entry();
    new_entries1[i]->init(my_tree);
    new_entries1[i]->level = level;
    new_entries2[i] = new_one_entry();
    new_entries2[i]->init(my_tree);
  ...
  for (i = 0; i < n; i++) {
    entries[i]->son_ptr = NULL;
    delete entries[i];
    _sn->entries[i]->son_ptr = NULL;
    delete _sn->entries[i];
  }
  delete[] entries;
  delete[] _sn->entries;
  //---
...
//	{
//		float range[2];
//		my_tree->get_score_range(&(entries[i]), _weight, dimension,
//range); //get the score range for this entry 		if (range[0]<=_qscore &&
//_qscore<=range[1])
//		{
