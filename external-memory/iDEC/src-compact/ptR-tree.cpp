...
// // #include <string.h>
// #include <stdlib.h>
// the order of those includes matters (PLEASE DO NOT CHANGE)
#include <stdio.h>
#include <stdlib.h>
#include <memory.h>
#include "ptR-tree.h"
...
int ptEntry::get_size() {
  if (level == -1) {
    printf("ptEntry::get_size detected that the level of an entry has not been "
           "properly set.\n");
    error("The most likely cause of this error is that you forgot to initiate "
          "the level of an entry after creating it. \n",
          true);
  }

  if (level == 0) {
    // return dimension * sizeof(float) + sizeof(int); //for bounces and son
    return dimension * sizeof(int16_t) + sizeof(int); // for bounces and son
  }
...
void ptEntry::read_from_buffer(char *_buffer) {
  if (level == -1) {
    printf("ptEntry::read_from_buffer detected that the level of an entry has "
  ...
  if (level == 0) {
    ...
    for (int j = 0; j < dimension; j++) {
      ...
      memcpy(&bounces[2 * j], &_buffer[i], sizeof(int16_t));
      i += sizeof(int16_t); // sizeof(float);
...
void ptEntry::write_to_buffer(char *_buffer) {
  if (level == -1) {
    printf("ptEntry::write_to_buffer detected that the level of an entry has "
           "not been properly set.\n");
    error("The most likely cause of this error is that you forgot to initiate "
          "the level of an entry after creating it. \n",
          true);
  ...
  if (level == 0) {
    ...
    for (int j = 0; j < dimension; j++) {
      ...
      memcpy(&_buffer[i], &bounces[2 * j], sizeof(int16_t));
      i += sizeof(int16_t); // sizeof(float);
    }
...
// initiates an entry. Override this function to create an object of a class
// that inherits Entry Coded by Yufei Tao 05/31/06
...
// inherits RTNode Coded by Yufei Tao 05/31/06
...
void ptRTree::fread_next_entry(FILE *_fp, Entry *_d) {
  if (fscanf(_fp, "%d\t", &(_d->son)) != 1) {
    perror("ptRTree::fread_next_entry failed");
  }
  for (int i = 0; i < dimension; i++) {
    // fscanf(_fp, "%f", &(_d->bounces[2*i]));
    if (fscanf(_fp, "%hd", &(_d->bounces[2 * i])) != 1) {
      perror("ptRTree::fread_next_entry failed");
    }
