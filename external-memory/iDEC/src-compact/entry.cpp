...
void Entry::init(RTree *_rt)
{
  if (_rt)
  {
    ...
    bounces = new int16_t[2 * dimension];
  }
  son = 0;
...
int Entry::get_size()
{
  ...
  return 2 * dimension * sizeof(int16_t) + sizeof(int); //for bounces and son
}
//------------------------------------------------------------
...
SECTION Entry::section(int16_t *mbr)
{
  bool inside;
  bool overlap;
...
void Entry::set_from_Linkable(Linkable *_link)
{
  ...
  memcpy(bounces, _link -> bounces, 2 * dimension * sizeof(int16_t));
  level = _link -> level;
...
void Entry::write_to_buffer(char *_buffer)
{
  ...
  int i = 2 * dimension * sizeof(int16_t);
...
bool Entry::check_equal(Entry *_d)
{
  ...
  for (int i = 0; i < 2 * dimension; i++) if (bounces[i] != _d->bounces[i]) return false;
...
void Entry::set_equal_to(Entry *_d)
{
  ...
  memcpy(bounces, _d->bounces, sizeof(int16_t) * 2 * dimension);
  my_tree = _d->my_tree;
  level = _d->level;
}
