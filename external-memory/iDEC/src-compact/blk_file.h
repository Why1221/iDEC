...
class BlockFile{
 public:
  ...
  bool new_flag;	 // specifies if this is a new file
   ...
   //=====================================================
  BlockFile(char* name, int b_length);// filename and block length
  ...
  void get_bytes(char* bytes,int num){
    if ((int)fread(bytes,num,1,fp) != 1) {
      fprintf(stderr, "get_bytes() from %s failed\n", filename);
      perror("ERROR");
    } 
  }
  ...
  void seek_block(int bnum){
    fseeko64(fp,(bnum-act_block)*blocklength,SEEK_CUR); 
  }
