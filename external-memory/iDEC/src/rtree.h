...
//--===added for cost summary===--
	...
	virtual ~RTree();		
	virtual void build_from_file(char *_dsname);	//construct a tree from a set of rectangles
	...
	virtual void init(char *_fname, int _b_length, Cache* _c, int _dimension);		//creates a new tree
	virtual void init(char *_fname, Cache* _c);		//loads an existing tree
	virtual void insert(Entry *_d);
	virtual int kNN(float *_q, int _k, Heap *_hp, float &_fardist);	//performs a kNN query
	...
	virtual int rangeQuery(float *_mbr, SortedLinList *_res, bool _ids_wanted);	//performs a range query
	virtual void read_header(char *_buffer);		//loads the parameters of the tree from the disk buffer page
	virtual void write_header(char *_buffer);		//writes the parameters of the tree into the disk buffer page
