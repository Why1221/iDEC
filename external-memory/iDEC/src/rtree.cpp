...
void RTree::close()
{
  ...
  if (cache)
  {
    cache -> flush(); //note that a cache object is not destroyed here. Remember that this object
    //was not initiated in the R-tree, but passed in when the tree was initiated.
  }

  delete file; file = NULL;
  delete re_data_cands; re_data_cands = NULL;
  delete deletelist; deletelist = NULL;
}
//------------------------------------------------------------
RTree::~RTree()
{
  if (file)
  {
    ...
    error("The most likely cause of this error is that you forgot to call .close before deleting the object", 
          true);
  }
  ...
  // printf("This R-Tree contains %d internal, %d data node(s) and %d object(s)\n", num_of_inodes, num_of_dnodes, 
  //        num_of_data);
}
//------------------------------------------------------------
//deletes an object
...
void RTree::insert(Entry* _d)
{
  ...
  delete _d;  //we follow the convention that the entry will be deleted when insertion finishes
