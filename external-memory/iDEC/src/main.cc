...
int build(char *_dfname, char * _ffolder, int _n, int _d, int _B, int _L, int _m) {
  int ret = 0;
  SRS * srs = NULL;
...
int ctbuild(char *_dfname, char * _ffolder, int _n, int _d, int _B, int _L, int _m) {
  int ret = 0;
  SRS * srs = NULL;
...
int srsknn(char *_ffolder, int _numTrees, char *_rfname, char *_qfname, int _k, char *_ofname, int _start_id, float _c, float _tau, float _t, bool early_stop) {
  int ret = 0;
  ...
  while (!feof(qfp)) {
    ...
    thiscost = srs->knn(q, _k, rslt, _start_id, _numTrees, _c, _tau, _t, early_stop);
...
void usage() {
  ...
  printf("-tau {value}\tthreshold of early termination condition (default 0.1809)\n");
  printf("-t {value}\tmaximum percentage of examed points (default 0.00242)\n");
  printf("-es [0|1]\t1: enable early stop; 0: disable early stop (default 1)\n");
...
int main(int argc, char** argv) {
  ...
  if (!failed) {
    if (dfname[0] != '\0' && ffolder[0] != '\0' && n != -1 && d != -1 && Bb != -1 && m != -1 && L != -1 && ct == 0) {
      build(dfname, ffolder, n, d, Bb / 4, L, m);
      goto end;
    } else if (dfname[0] != '\0' && ffolder[0] != '\0' && n != -1 && d != -1 && Bb != -1 && m != -1 && L != -1 && ct == 1) {
      ctbuild(dfname, ffolder, n, d, Bb / 4, L, m);
      goto end;
    } 
    if (dfname[0] != '\0' && ffolder[0] != '\0' && n != -1 && d != -1 && Bb != -1) {
      ...
      srsknn(ffolder, nt, rfname, qfname, k, ofname, startID, ratio, tau, t, early_stop);
      goto end;
    } else {
      failed = true;
    }
